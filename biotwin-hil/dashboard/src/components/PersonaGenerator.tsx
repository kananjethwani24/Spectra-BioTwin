...
// Accurate clinical mock data for each condition
...
export default function PersonaGenerator({ onPersonaApplied, onRemovePersona, isPersonaApplied, twinState }: Props) {
  const [condition, setCondition] = useState("Atrial Fibrillation");
  const [persona, setPersona] = useState<PersonaPayload | null>(null);
  const [loading, setLoading] = useState(false);

  const generate = () => {
    setLoading(true);
    onRemovePersona(); // Reset application to twin when generating a new scenario
    setTimeout(() => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
            Clinical mock data · 5 conditions
          </p>
        </div>
