...
export type SensorMode = "SIMULATION_MODE" | "REAL_SENSOR_MODE" | "NORMAL_MODE";
...
export function useTwinState(wsUrl: string) {
    ...
    ws.onmessage = (event: MessageEvent) => {
      try {
        ...
        if (packet.type === "SIGNAL_DATA") {
          ...
          if (sensorModeRef.current === "REAL_SENSOR_MODE" && !isPersonaAppliedRef.current) {
  ...
  const applyPersona = useCallback((persona: PersonaPayload) => {
    ...
    const temp = rand(cv.tempMin, cv.tempMax);
  ...
  const applySensorData = useCallback((data: RawSensorData) => {
    setTwinState((prev) => {
      ...
      const calculatedHR = addHRSample(rawIR, Date.now());
      const hr = calculatedHR > 0 ? calculatedHR : (data.heart_rate ?? 0);
      // Calculate SpO2 from IR/RED waveforms, fall back to library value
      const calculatedSpO2 = addSpO2Sample(rawIR, rawRed);
      const spo2 = calculatedSpO2 > 0 ? calculatedSpO2 : (data.spo2 ?? 98);
      const temp = data.temperature ?? 36.5;
  ...
  const handleSetSensorMode = useCallback((mode: SensorMode) => {
    ...
    if (mode === "NORMAL_MODE") {
      ...
      setTwinState({
        patientId: "default-patient",
        age: 35, gender: "Unknown", weight: 70,
        condition: "Normal", arrhythmia: false,
        heartRate: hr, spo2, temperature: temp,
        humidity: 40, pressure: 1013.25,
        ...scores,
        conditionProbabilities: classifyCondition(hr, spo2, temp),
        sensorMode: mode,
        spectralStatus: "disabled", spectralChannels: [],
        ir: 0, red: 0,
      });
    } else {
